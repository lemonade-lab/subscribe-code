...
const ALERT_TOKEN: Array<{ chatId: string; token: string; type: 'message.create' | 'private.message.create' }> =
    config?.alert_token || [];
...
const POST = async ctx => {
    try {
        ...
        try {
            ...
            logger.error(
                chalk.bgRed.white('[Bot warning report]'),
        ...
        if (verifyToken && ALERT_TOKEN.some(item => item.token === verifyToken)) {
            for (const chatSummary of ALERT_TOKEN) {
                if (chatSummary.token === verifyToken) {
                    ...
                    await sendMessage(
                        chatSummary.type === 'message.create' ? 'message.create' : 'private.message.create',
                        chatSummary.chatId,
                        alertMessage
                    );
                    ctx.status = 200;
            ...
            logger.error(
                chalk.bgRed.white('[Alert Warning]'),
                chalk.red('请求header中携带的token未找到匹配的开发群组：'),
                verifyToken
            );
        }
    } catch (err) {

...
export const regular =
    /^(\/code|!|！)m\s+(-ga|ga)\s+opt\s+([a-zA-Z0-9_-]{1,39}\/[a-zA-Z0-9._-]{1,39}\s*(:|：)\s*[a-zA-Z0-9_-]\s*(:|：)[a-zA-Z0-9_-])?$/;
...
const res = onResponse(selects, async e => {
    ...
    if (match && match[3]) {
        ...
        if (!thisWorkflowsListData) {
            message.send(format(Text('没有找到此仓库')));
            return;
        }
        ...
        const response = await fetch(
            `https://api.github.com/repos/${userRepoName}/actions/workflows/${thisRepoWorkFlowData.pathFile}/dispatches`,
            {
                method: 'POST',
                headers: {

...
export const regular = /^(\/code|!|！)\s+add\s+(-g\s+)?(https?:\/\/)?(www\.)?(github\.com\/)?([a-zA-Z0-9_.-]+)\/([a-zA-Z0-9_.-]+)(\s+-g)?(\s|$)/;
...
const res = onResponse(selects, async e => {
  ...
  if (e.name === 'message.create') {
    ...
    if (exist) {
      void message.send(format(Text(`该群聊已经订阅过该仓库：${repo.belong}/${repo.name}`)));

      return;
    }
    await CodeData.add(repo.origin, repo.belong, repo.name, 'g', e.SpaceId);
    void message.send(format(Text(`订阅成功：${repo.belong}/${repo.name}`)));
  ...
  if (e.name === 'private.message.create') {
    ...
    if (exist) {
      void message.send(format(Text(`你已经订阅过该仓库：${repo.belong}/${repo.name}`)));

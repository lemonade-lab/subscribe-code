...
export const regular = Regular.or(addAdminReg, delAdminReg, canPriviteSubscribeReg, noPriviteSubscribeReg);
...
export default onResponse(selects, async e => {
    ...
    if (canPriviteSubscribeReg.test(e.MessageText)) {
        ...
        const canPrivateSubscribeUsers: string[] = configValue?.['alemonjs-code']?.can_private_subscribe_users || [];
        ...
        enhancedConfigUpdate(
            'alemonjs-code',
            { can_private_subscribe_users: [...canPrivateSubscribeUsers, newCanSubscribeUser] },
    ...
    if (noPriviteSubscribeReg.test(e.MessageText)) {
        ...
        let canPrivateSubscribeUsers: string[] = configValue?.['alemonjs-code']?.can_private_subscribe_users || [];
        if (!canPrivateSubscribeUsers.includes(notSubscribeUser)) {
            message.send(format(Text('该账号未被授权私信订阅')));
            return;
        }
        canPrivateSubscribeUsers = canPrivateSubscribeUsers.filter((id: string) => id !== notSubscribeUser);
        message.send(format(Text(`已取消授权该用户私信订阅: ${notSubscribeUser}`)));
        enhancedConfigUpdate(
            'alemonjs-code',
            { can_private_subscribe_users: canPrivateSubscribeUsers },
            { merge: true, unique: true }
        );
    }
    return;

...
export function isCodeMaster(userKey: string, userId: string): boolean {
 ...
 */
export function addAlertToken(alertTokenItem: { chatId: string; token: string; type: 'message.create' | 'private.message.create' }): boolean {
  const config = getConfig();
  const val = config.value;
...
export function listAlertChats(): {
  ...
  // 遍历所有alert_token项并分类
  val[appName][keyName].forEach((item: { chatId: string; token: string; type: 'message.create' | 'private.message.create' }) => {
 ...
 */
export function removeAlertChat(alertChatIdItem: { chatId: string; type: 'message.create' | 'private.message.create' }): boolean {
  const config = getConfig();
  const val = config.value;
  const keyName = 'alert_token';
...
export function addActionToken(actionTokenItem: {
  ...
  const existingIndex = val[appName][keyName].findIndex(
    (item: { userRepoName: string; token: string; workflows: Array<{ name: string; branch: string; pathFile: string; url: string }> }) =>

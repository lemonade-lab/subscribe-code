...
// 根据仓库和聊天ID查找订阅
export const findByRepoAndChatId = async (origin: string, belong: string, name: string, type: 'g' | 'w', chatId: string): Promise<string | null> => {
  const ioredis = getIoRedis();
...
// 根据仓库查找所有订阅
export const findByRepo = async (origin: string, belong: string, name: string, activeOnly?: boolean): Promise<DataType[]> => {
  const ioredis = getIoRedis();
...
// 复合查询：仓库 + 类型
export const findByRepoAndType = async (origin: string, belong: string, name: string, type: 'g' | 'w', activeOnly?: boolean): Promise<DataType[]> => {
  const ioredis = getIoRedis();
  const repoIndexKey = `${indexPrefix}:repo:${origin}/${belong}/${name}`;
  const typeIndexKey = `${indexPrefix}:type:${type}`;

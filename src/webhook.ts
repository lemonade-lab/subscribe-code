...
export const createWebhookServer = async (options: { port?: number; githubSecret?: string; wsSecret?: string }) => {
    ...
    // WebSocket 客户端（中转模式，ws_secret 可选），WebSocketServer 监听同一端口
    const wss = new WebSocketServer({ server });

    wss.on('connection', ws => {
        const challenge = randomUUID();
        //服务端配置了wsSecret，下发challenge给客户端，客户端需同步配置wsSecret并返回签名认证
        if (WS_SECRET) {
                ...
                if (!authed) {
                    if (WS_SECRET) {
                        ...
                        logger.warn(
                            ...
                            chalk.yellow('未配置ws_secret，跳过认证，分配客户端Id:'),
                            chalk.bold(clientId),
                            chalk.yellow('当前连接数:'),
                            clients.size
                        );

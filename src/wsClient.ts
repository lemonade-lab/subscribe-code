...
// 读取 ws_secret 和 ws_server_url
let WS_SECRET = configValue?.['alemonjs-code']?.ws_secret;
if (!WS_SECRET) {
    WS_SECRET = '请填入github webhook中转服务生成的的ws_secret';
    updateConfig('alemonjs-code', { ws_secret: WS_SECRET });
    console.log(
    ...
    console.log(chalk.bgGreen.black('[WebSocket Client]'), chalk.green('已读取ws_secret'), chalk.yellow(WS_SECRET));
}
...
if (!WS_SERVER_URL) {
    ...
    console.log(
        chalk.bgYellow.black('[WebSocket Client]'),
        chalk.yellow('未设置ws_server_url，已自动设置为默认:'),
        chalk.bold(WS_SERVER_URL)
    );
} else {
    console.log(
        chalk.bgGreen.black('[WebSocket Client]'),
        chalk.green('已读取ws_server_url'),
        chalk.yellow(WS_SERVER_URL)
    );
}

...
const WebsokcetClient = async (options) => {
    ...
    function connectWS() {
        ...
        const fingerPrint = crypto
            ...
            .digest('hex');
        ws.on('open', () => {
            logger.info(chalk.bgBlue.white('[WebSocket Client]'), chalk.blue('已连接到服务端:'), chalk.bold(`${WS_SERVER_URL}`));
            heartbeat();
        });
            ...
            try {
                ...
                for (const sub of subs) {
                    ...
                    if (!SubscriptionService.isAllSubscriptionsEnabled(subs.filter(s => s.id === subId))) {
                        logger.info(chalk.bgYellow.black('[WebSocket Client]'), chalk.yellow('该聊天的推送已暂停，跳过发送：'), `[${sub.poolType}] [${sub.chatId}]`, chalk.gray(repo));
                        continue;
                    }
                    if (sub.status === SubscriptionStatus.Disabled) {
                        logger.info(chalk.bgYellow.black('[WebSocket Client]'), chalk.yellow('该编号仓库推送已暂停，跳过发送：'), `[${sub.poolType}] [${sub.chatId}] [${subId}]`, chalk.gray(repo));
                        continue;
                    }

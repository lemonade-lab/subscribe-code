...
const removeSubByUrlReg = /^(!|！|\/)?(移除|取消|删除|del|DEL|delete|DELETE)(本聊天)?(仓库|github仓库|GitHub仓库|GitHub代码仓库)\s*(https?:\/\/)?github\.com\/[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_.-]+/;
const removeByIdReg = /^(!|！|\/)?(移除|取消|删除|del|DEL|delete|DELETE)编号仓库\s*([a-z0-9]{8})$/i;
const removeRepoPoolRegex = /^([!！/])?(移除|取消|删除|del|DEL|delete|DELETE)(仓库池|github仓库池|GitHub代码仓库池)\s*(https?:\/\/)?(github\.com\/)?[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_.-]+/;
const regular = Regular.or(removeSubByUrlReg, removeByIdReg, removeRepoPoolRegex);

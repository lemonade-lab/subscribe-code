...
import PermissionService, { UserRole } from '../../models/github.sub.permissoin.js';

const regular = /^(!|！|\/)?(仓库帮助|code-help|codeh)$/;
const helpData = {
    data: [
        {
            group: '公共指令',
...
var res = onResponse(selects, async (e) => {
    ...
    const rolePermissions = {
        公共指令: () => true,
        私聊订阅管理: () => true,
        仓库池管理: (r) => [UserRole.Master, UserRole.GlobalCodeMaster, UserRole.CodeMaster].includes(r),
        群组订阅管理: (r) => [UserRole.Master, UserRole.GlobalCodeMaster, UserRole.CodeMaster].includes(r),
        跨群组管理: (r) => [UserRole.Master, UserRole.GlobalCodeMaster].includes(r),
        权限管理: (r) => r === UserRole.Master
    };

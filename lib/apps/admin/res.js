...
import PermissionService, { UserRole, Action } from '../../models/github.sub.permissoin.js';
...
const addGlobalAdminReg = /^([!|！|/])?(新增全局仓库管理员|添加全局仓库管理员|codegm-add|codegm-a)\s*([a-zA-Z0-9_.-]+)?$/;
const delGlobalAdminReg = /^([!|！|/])?(删除全局仓库管理员|移除全局仓库管理员|codegm-del|codegm-d)\s*([a-zA-Z0-9_.-]+)?$/;
const removeAdminAllRolesReg = /^([!|！|/])?(删除仓库管理员角色|移除仓库管理员角色|codem-del|codem-d)\s*([a-zA-Z0-9_.-]+)?$/;
...
const regular = Regular.or(addAdminReg, delAdminReg, addGlobalAdminReg, delGlobalAdminReg, removeAdminAllRolesReg, listAllAdminsReg, cleanAllAdminsReg, canPriviteSubscribeReg, noPriviteSubscribeReg, listAllPriviteSubsReg, myPermissionReg);
var res = onResponse(selects, async (e) => {
    ...
    if (removeAdminAllRolesReg.test(e.MessageText)) {
        ...
        if (!((await PermissionService.getUserRole(delUserAdmin)) === UserRole.GlobalCodeMaster) &&
    ...
    if (listAllAdminsReg.test(e.MessageText)) {
        ...
        if (allAdmins.length > 0) {
            for (const admin of allAdmins) {
                msg.push(`${admin.userKey} : ${admin.role === UserRole.GlobalCodeMaster ? '全局管理员' : admin.role === UserRole.CodeMaster ? '群管理员' : '普通用户'}`);
            }
        }
        else {
            msg.push('暂无管理员');

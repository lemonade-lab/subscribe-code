...
import { SubscriptionStatus } from './models/github.sub.permissoin.js';
import { keyHashData } from './utils/config.js';
...
const WebhookWebsocketServer = async (options) => {
        ...
        try {
            ...
            for (const sub of subs) {
                ...
                if (!SubscriptionService.isAllSubscriptionsEnabled(subs.filter(s => s.id === subId))) {
                    logger.info(chalk.bgYellow.black('[GitHub Webhook]'), chalk.yellow('该聊天的推送已暂停，跳过发送：'), `[${sub.poolType}] [${sub.chatId}]`, chalk.gray(repo));
                    continue;
                }
                if (sub.status === SubscriptionStatus.Disabled) {
                    logger.info(chalk.bgYellow.black('[GitHub Webhook]'), chalk.yellow('该编号仓库推送已暂停，跳过发送：'), `[${sub.poolType}] [${sub.chatId}] [${subId}]`, chalk.gray(repo));
                    continue;
                }
    ...
    const server = app.listen(PORT, () => {
        logger.info(chalk.bgBlue.white('[Webhook Server]'), chalk.blue('服务已启动:'), chalk.bold(`http://localhost:${PORT}`));
        logger.info(chalk.bgMagenta.white('[WebSocket Server]'), chalk.magenta('WebSocket 客户端连接地址:'), chalk.bold(`ws://localhost:${PORT}`));
    });
